...
const industry = (
  ...
  questions: string[],
  current_flow: string,
  safety_note: string | null = null,
  future_flow: string | null = null,
): Industry => ({ label, slug, description, questions, current_flow, safety_note, future_flow });

export const businessTaxonomy: BusinessType[] = [
  {
    label: 'Salon / Beauty',
    slug: 'salon-beauty',
    default_mode: 'appointment',
    ...
    safety_copy: null,
    industries: [
      industry('Hair salon', 'hair-salon', 'YouGo helps hair salons answer client questions, explain services, show prices, manage appointment requests and reduce missed calls while stylists are busy with clients.', ['Do you have availability today?', 'How much is a haircut or colouring?', 'Can I book with a specific stylist?', 'Where are you located?'], 'service -> date -> time -> preferred stylist -> client details -> appointment request'),
      industry('Nail salon', 'nail-salon', 'YouGo helps nail salons handle manicure, pedicure, gel, extensions and maintenance appointment requests through chat, WhatsApp or phone in the future.', ['Do you do gel nails?', 'How much is infill?', 'Can I book for Friday?', 'Which technician is available?'], 'service -> preferred technician -> date -> time -> client details'),
  ...
  },
  {
    ...
    safety_copy: null,
    industries: [
      industry('Car repair', 'car-repair', 'YouGo helps car repair garages collect useful vehicle and issue details before a booking request reaches the team.', ['My car makes a noise, can you check it?', 'Can I book a repair?', 'How much is diagnostics?'], 'problem -> vehicle make/model/year -> preferred date -> client details -> appointment request'),
      industry('MOT / inspection', 'mot-inspection', 'YouGo helps garages handle MOT or inspection appointment requests, opening hours and availability questions.', ['Can I book an MOT?', 'How much is an inspection?', 'Do you have availability this week?'], 'service type -> vehicle details -> date -> time -> client details'),
      industry('Tyres', 'tyres', 'YouGo helps tyre shops answer questions about tyre replacement, fitting, availability and appointment requests.', ['Do you fit tyres?', 'Can I book tyre replacement?', 'What tyre size do you need?'], 'tyre issue/service -> vehicle/tyre size -> date -> time -> client details'),
      industry('Detailing', 'detailing', 'YouGo helps car detailing businesses explain packages, prices, duration and collect booking requests.', ['What detailing packages do you offer?', 'How long does it take?', 'Can I book for Saturday?'], 'package -> vehicle type -> date -> time -> client details'),
      industry('Car diagnostics', 'car-diagnostics', 'YouGo helps diagnostic garages collect symptoms, warning light details, vehicle information and preferred appointment times.', ['My engine light is on, can you check it?', 'How much is diagnostics?', 'When can I bring the car?'], 'symptom -> vehicle details -> urgency -> date -> time -> client details'),
    ],
  },
  {
    ...
    industries: [
      ...
      industry('Legal services', 'legal-services', 'YouGo helps legal service providers collect initial enquiry details and route appointment requests without giving legal advice.', ['Can I speak with a solicitor?', 'Do you handle family/property/business matters?', 'How do I book a consultation?'], 'matter type -> urgency -> preferred consultation time -> contact details', 'AI should not provide legal advice. It should collect details and offer an appointment or handover.'),
      industry('Consulting', 'consulting', 'YouGo helps consultants qualify enquiries, understand business needs and collect contact details for a consultation.', ['Can I book a consultation?', 'Do you work with small businesses?', 'What services do you offer?'], 'business need -> service area -> preferred time -> contact details'),
      industry('Insurance', 'insurance', 'YouGo helps insurance brokers collect enquiry details, understand what type of cover is needed and arrange follow-up.', ['Can I get a quote?', 'What insurance do I need?', 'Can someone call me back?'], 'insurance type -> basic details -> preferred contact time -> contact details'),
      industry('Financial advice', 'financial-advice', 'YouGo helps financial advice firms collect consultation requests and basic enquiry details without providing regulated financial advice.', ['Can I book a consultation?', 'Do you help with mortgages/pensions/investments?', 'What are your fees?'], 'enquiry area -> consultation preference -> contact details', 'AI should not provide regulated financial advice. It should collect details and arrange a human follow-up.'),
    ],
  },
  {
    label: 'Restaurant',
    ...
    industries: [
      ...
      industry('Cafe', 'cafe', 'YouGo helps cafes answer questions about opening hours, menu, events and booking enquiries.', ['Are you open today?', 'Do you take bookings?', 'Do you serve breakfast?'], 'enquiry -> date/time preference -> contact details'),
      industry('Bar', 'bar', 'YouGo helps bars answer event, opening hour, table enquiry and private booking questions.', ['Can I book a table?', 'Are you open late?', 'Do you host private events?'], 'enquiry -> party size/date/time -> contact details'),
      industry('Catering', 'catering', 'YouGo helps catering businesses collect event enquiries, dates, guest numbers, menu preferences and contact details.', ['Do you cater weddings?', 'Can I get a quote?', 'Are you available on this date?'], 'event type -> date -> guest count -> menu needs -> contact details'),
    ],
  },
  {
    ...
    safety_copy: null,
    industries: [
      industry('Hotel', 'hotel', 'YouGo helps hotels answer guest questions and collect booking enquiries. Full room availability and reservation management will be added later.', ['Do you have rooms available?', 'What is the price per night?', 'Is breakfast included?'], 'check-in date -> nights -> guests -> room preference -> contact details', null, 'dates -> guests -> room availability -> booking'),
      industry('Guest house', 'guest-house', 'YouGo helps guest houses answer availability, pricing, location and facility questions while collecting booking enquiries.', ['Do you have availability this weekend?', 'Is parking available?', 'Can I book directly?'], 'date -> nights -> guests -> contact details'),
      industry('B&B', 'b-and-b', 'YouGo helps B&Bs handle guest enquiries about availability, breakfast, check-in, rooms and location.', ['Is breakfast included?', 'Do you have a double room?', 'What time is check-in?'], 'date -> nights -> guests -> room preference -> contact details'),
      industry('Holiday rental', 'holiday-rental', 'YouGo helps holiday rental businesses collect stay enquiries, dates, number of guests and property preferences.', ['Is the property available in August?', 'How many guests can stay?', 'Is there parking?'], 'property/stay enquiry -> dates -> guests -> contact details'),
    ],
  },
  {
    label: 'Rental',
    ...
    safety_copy: null,
    industries: [
      industry('Car rental', 'car-rental', 'YouGo helps car rental businesses collect vehicle rental enquiries, dates, vehicle type, driver details and contact information. Full reservation logic will be added later.', ['Do you have a car available next week?', 'How much is a van for two days?', 'Can I rent an automatic car?'], 'vehicle type -> start date -> end date -> contact details', null, 'vehicle/resource -> start date -> end date -> availability -> reservation'),
      industry('Equipment rental', 'equipment-rental', 'YouGo helps equipment rental businesses collect enquiries about machines, tools or equipment, required dates and customer details.', ['Do you rent this equipment?', 'Is it available next weekend?', 'How much per day?'], 'equipment type -> rental period -> contact details'),
      industry('Tool rental', 'tool-rental', 'YouGo helps tool rental businesses answer availability and price enquiries and collect hire requests.', ['Do you hire drills or saws?', 'Can I rent it for one day?', 'What deposit is required?'], 'tool type -> rental date/duration -> contact details'),
      industry('Event rental', 'event-rental', 'YouGo helps event rental businesses collect enquiries for furniture, decorations, equipment, event dates and delivery details.', ['Do you rent chairs and tables?', 'Are you available for my event date?', 'Can you deliver?'], 'event type -> rental items -> event date -> contact details'),
      industry('Property rental', 'property-rental', 'YouGo helps property rental businesses collect tenant or short-stay enquiries, property preferences, dates and contact details.', ['Is this property available?', 'Can I arrange a viewing?', 'What is the rent?'], 'property interest -> date/viewing request -> contact details'),
    ],
  },
  {
    label: 'Real estate',
    ...
    safety_copy: null,
    industries: [
      industry('Property sales', 'property-sales', 'YouGo helps estate agents collect buyer enquiries, property interests, viewing requests and contact details.', ['Is this property still available?', 'Can I book a viewing?', 'What is the asking price?'], 'property interest -> viewing date/time -> contact details', null, 'buy/sell intent -> budget/property -> location -> contact details -> lead'),
      industry('Property lettings', 'property-lettings', 'YouGo helps letting agents collect tenant enquiries, viewing requests and property requirements.', ['Is this flat available?', 'Can I book a viewing?', 'What documents do I need?'], 'property interest -> viewing request -> contact details'),
      industry('Property management', 'property-management', 'YouGo helps property management companies handle tenant, landlord and maintenance enquiries and route them to the right team.', ['I need to report a repair.', 'Can someone contact me?', 'Do you manage properties in this area?'], 'enquiry type -> property/address -> urgency -> contact details'),
    ],
  },
  {
    label: 'Other',
    ...
    safety_copy: null,
    industries: [
      industry('Other', 'other', 'YouGo can be configured for other appointment-style businesses by adding services, locations, staff and AI instructions in the dashboard.', ['What services do you offer?', 'Can I book a time?', 'Where are you located?'], 'enquiry -> service -> date/time -> contact details'),
    ],
  },
];

export function findBusinessType(slug?: string | null): BusinessType | undefined {
  return businessTaxonomy.find((item) => item.slug === slug);
}

export function findIndustry(businessTypeSlug?: string | null, industrySlug?: string | null): Industry | undefined {
  return findBusinessType(businessTypeSlug)?.industries.find((item) => item.slug === industrySlug);
}

export function normalizeBusinessTypeSlug(value?: string | null): string {
  if (!value) return '';
  const normalized = value.trim().toLowerCase();
  return businessTaxonomy.find((item) => item.slug === value || item.label.toLowerCase() === normalized)?.slug ?? '';
}

export function normalizeIndustrySlug(value?: string | null, businessTypeSlug?: string | null): string {
  if (!value) return '';
  const normalized = value.trim().toLowerCase();
  const groups = businessTypeSlug ? [findBusinessType(businessTypeSlug)].filter(Boolean) as BusinessType[] : businessTaxonomy;
