...
class BusinessTaxonomy
{
    ...
    private static function rawBusinessTypes(): array
    {
        return [
            ...
            [
                ...
                'safety_copy' => null,
                'industries' => [
                    self::industry('Car repair', 'car-repair', 'YouGo helps car repair garages collect useful vehicle and issue details before a booking request reaches the team.', ['My car makes a noise, can you check it?', 'Can I book a repair?', 'How much is diagnostics?'], 'problem -> vehicle make/model/year -> preferred date -> client details -> appointment request', labelRo: 'Reparații auto'),
                    self::industry('MOT / inspection', 'mot-inspection', 'YouGo helps garages handle MOT or inspection appointment requests, opening hours and availability questions.', ['Can I book an MOT?', 'How much is an inspection?', 'Do you have availability this week?'], 'service type -> vehicle details -> date -> time -> client details', labelRo: 'ITP / Inspecție tehnică'),
                    self::industry('Tyres', 'tyres', 'YouGo helps tyre shops answer questions about tyre replacement, fitting, availability and appointment requests.', ['Do you fit tyres?', 'Can I book tyre replacement?', 'What tyre size do you need?'], 'tyre issue/service -> vehicle/tyre size -> date -> time -> client details', labelRo: 'Anvelope'),
                    self::industry('Detailing', 'detailing', 'YouGo helps car detailing businesses explain packages, prices, duration and collect booking requests.', ['What detailing packages do you offer?', 'How long does it take?', 'Can I book for Saturday?'], 'package -> vehicle type -> date -> time -> client details', labelRo: 'Detailing auto'),
                    self::industry('Car diagnostics', 'car-diagnostics', 'YouGo helps diagnostic garages collect symptoms, warning light details, vehicle information and preferred appointment times.', ['My engine light is on, can you check it?', 'How much is diagnostics?', 'When can I bring the car?'], 'symptom -> vehicle details -> urgency -> date -> time -> client details', labelRo: 'Diagnoză auto'),
                ],
            ],
            [
                'label' => 'Home & trade services',
                'label_ro' => 'Servicii casnice și meserii',
                'slug' => 'home-services',
                'default_mode' => Salon::MODE_APPOINTMENT,
                'future_mode' => null,
            ...
            [
                ...
                'industries' => [
                    ...
                    self::industry('Cafe', 'cafe', 'YouGo helps cafes answer questions about opening hours, menu, events and booking enquiries.', ['Are you open today?', 'Do you take bookings?', 'Do you serve breakfast?'], 'enquiry -> date/time preference -> contact details', labelRo: 'Cafenea'),
                    self::industry('Bar', 'bar', 'YouGo helps bars answer event, opening hour, table enquiry and private booking questions.', ['Can I book a table?', 'Are you open late?', 'Do you host private events?'], 'enquiry -> party size/date/time -> contact details', labelRo: 'Bar'),
            ...
            [
                ...
                'safety_copy' => null,
                'industries' => [
                    self::industry('Hotel', 'hotel', 'YouGo helps hotels answer guest questions and collect booking enquiries. Full room availability and reservation management will be added later.', ['Do you have rooms available?', 'What is the price per night?', 'Is breakfast included?'], 'check-in date -> nights -> guests -> room preference -> contact details', null, 'dates -> guests -> room availability -> booking', labelRo: 'Hotel'),
                    self::industry('Guest house', 'guest-house', 'YouGo helps guest houses answer availability, pricing, location and facility questions while collecting booking enquiries.', ['Do you have availability this weekend?', 'Is parking available?', 'Can I book directly?'], 'date -> nights -> guests -> contact details', labelRo: 'Pensiune'),
                    self::industry('B&B', 'b-and-b', 'YouGo helps B&Bs handle guest enquiries about availability, breakfast, check-in, rooms and location.', ['Is breakfast included?', 'Do you have a double room?', 'What time is check-in?'], 'date -> nights -> guests -> room preference -> contact details', labelRo: 'Cazare la particular (B&B)'),
                    self::industry('Holiday rental', 'holiday-rental', 'YouGo helps holiday rental businesses collect stay enquiries, dates, number of guests and property preferences.', ['Is the property available in August?', 'How many guests can stay?', 'Is there parking?'], 'property/stay enquiry -> dates -> guests -> contact details', labelRo: 'Cazare de vacanță'),
                ],
            ],
            [
                'label' => 'Rental',
                ...
                'safety_copy' => null,
                'industries' => [
                    self::industry('Car rental', 'car-rental', 'YouGo helps car rental businesses collect vehicle rental enquiries, dates, vehicle type, driver details and contact information. Full reservation logic will be added later.', ['Do you have a car available next week?', 'How much is a van for two days?', 'Can I rent an automatic car?'], 'vehicle type -> start date -> end date -> contact details', null, 'vehicle/resource -> start date -> end date -> availability -> reservation', labelRo: 'Închirieri auto'),
                    self::industry('Equipment rental', 'equipment-rental', 'YouGo helps equipment rental businesses collect enquiries about machines, tools or equipment, required dates and customer details.', ['Do you rent this equipment?', 'Is it available next weekend?', 'How much per day?'], 'equipment type -> rental period -> contact details', labelRo: 'Închirieri echipamente'),
                    self::industry('Tool rental', 'tool-rental', 'YouGo helps tool rental businesses answer availability and price enquiries and collect hire requests.', ['Do you hire drills or saws?', 'Can I rent it for one day?', 'What deposit is required?'], 'tool type -> rental date/duration -> contact details', labelRo: 'Închirieri unelte'),
                    self::industry('Event rental', 'event-rental', 'YouGo helps event rental businesses collect enquiries for furniture, decorations, equipment, event dates and delivery details.', ['Do you rent chairs and tables?', 'Are you available for my event date?', 'Can you deliver?'], 'event type -> rental items -> event date -> contact details', labelRo: 'Închirieri pentru evenimente'),
                    self::industry('Property rental', 'property-rental', 'YouGo helps property rental businesses collect tenant or short-stay enquiries, property preferences, dates and contact details.', ['Is this property available?', 'Can I arrange a viewing?', 'What is the rent?'], 'property interest -> date/viewing request -> contact details', labelRo: 'Închirieri imobiliare'),
                ],
            ],
            [
                'label' => 'Real estate',
            ...
            [
                ...
                'safety_copy' => null,
                'industries' => [
                    self::industry('Other', 'other', 'YouGo can be configured for other appointment-style businesses by adding services, locations, staff and AI instructions in the dashboard.', ['What services do you offer?', 'Can I book a time?', 'Where are you located?'], 'enquiry -> service -> date/time -> contact details', labelRo: 'Altele'),
                ],
            ],
        ];
    }
     ...
     */
    private static function industry(string $label, string $slug, string $description, array $questions, string $currentFlow, ?string $safetyNote = null, ?string $futureFlow = null, ?array $capabilitiesOverride = null, ?string $labelRo = null): array
    {
        return [
            'label' => $label,
            'label_ro' => $labelRo ?? $label,
            'slug' => $slug,
