...
- WhatsApp dashboard conversations are separated by booking flow. A pending WhatsApp booking can be cancelled automatically by clear customer intent. Existing bookings are not edited or rescheduled by AI; customers are directed to phone handoff for changes. Confirmed, completed, cancelled, and archived bookings close the operational dashboard conversation; later courtesy messages can remain in the old transcript, while later operational booking messages start a new dashboard conversation. Website Chat also supports automatic cancellation for pending AI-created bookings, while keeping `+` only for new/separate booking flows. The old amendment/change-request cards, emails, and "Change resolved" workflow are deprecated. Dashboard -> Conversations remains transcript-only for this workflow.
